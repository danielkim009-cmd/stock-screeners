"""
Point-in-time S&P 500 composition from Wikipedia.
...
Cache: 24-hour TTL, same pattern as universes.py.
"""
...
def _fetch_wiki_tables() -> list[pd.DataFrame]:
    """Fetch and return all tables from the Wikipedia S&P 500 page."""
    resp = httpx.get(_WIKI_URL, headers=_WIKI_HEADERS, timeout=30, follow_redirects=True)
    resp.raise_for_status()
    return pd.read_html(StringIO(resp.text))


def _fetch_sp500_current() -> list[str]:
    """Scrape current S&P 500 constituents from Wikipedia (table 0)."""
    cache_name = "sp500_wiki_current"
...
def _fetch_sp500_changes() -> list[dict]:
    """Scrape S&P 500 historical changes from Wikipedia (table 1).

    Returns a list of dicts sorted by date descending:
        [{"date": "2024-03-18", "added": "SMCI", "removed": "WYNN"}, ...]
    """
    cache_name = "sp500_wiki_changes"
    if _cache_is_fresh(cache_name):
        return json.loads(_cache_path(cache_name).read_text())
...
def get_sp500_at_date(target_date: str) -> list[str]:
    """Reconstruct the S&P 500 composition at a given date.
    ...
    """
    current = set(_fetch_sp500_current())
...
def get_sp500_pit_data(start_date: str, end_date: str) -> tuple[list[str], list[str], list[dict]]:
    ...
    initial = set(_fetch_sp500_current())
    for ch in changes:
